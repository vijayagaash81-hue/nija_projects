...
// === CHECK FOR EXISTING CHILD ORDER BEFORE CREATING ===
if (createChildOrder && type7LineData) {
    log.debug('Checking for Existing Child Order', 'Parent ID: ' + recordId);
    
    // Get header values for duplicate check: subsidiary, project, division
    var parentSubsidiary = productionOrder.getValue('custrecord_njt_subsidiar');
    var parentProject = productionOrder.getValue('custrecord_njt_project_2');
    var parentDivision = productionOrder.getValue('custrecord_njt_pro_ord_devision');
    
    log.debug('Header Values for Duplicate Check', {
        subsidiary: parentSubsidiary,
        project: parentProject,
        division: parentDivision
    });
    
    // Search for existing child order with same subsidiary + project + division + parent reference
    var existingChildSearch = search.create({
        ...
        filters: [
            ...
            ['custrecord_njt_pro_ord_devision', 'anyof', parentDivision], 'AND',
    ...
    var existingChildOrderId = existingResult ? existingResult.id : null;
    
    log.debug('Existing Child Order Search', {
        existingId: existingChildOrderId,
        parentId: recordId,
        subsidiary: parentSubsidiary,
        project: parentProject,
        division: parentDivision
    ...
    var childOrderId = null;
    
    if (existingChildOrderId) {
        // EXISTING CHILD ORDER FOUND - USE IT
        childOrderId = existingChildOrderId;
        log.audit('EXISTING Child Order FOUND', 'Using existing Child ID: ' + childOrderId);
    } else {
        // NO EXISTING - CREATE NEW CHILD ORDER
        ...
        // Save NEW Child Production Order
        childOrderId = childProdOrder.save({
            enableSourcing: true,
            ignoreMandatoryFields: true
        });
        log.debug('10. NEW Child Production Order CREATED Successfully', childOrderId);
    }
    
    // === ALWAYS UPDATE PARENT SUBLIST WITH CHILD REFERENCE (NEW OR EXISTING) ===
    if (childOrderId) {
        log.debug('11. Updating Parent Sublist with SFG Reference', childOrderId);
        ...
        log.debug('WARNING: No Child Order ID available for sublist update');
    }
}
