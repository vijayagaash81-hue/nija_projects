...
define(['N/ui/dialog', 'N/currentRecord', 'N/url', 'N/record'], function (dialog, currentrecord, url, record) {
    ...
    function callPickListSuitelet() {
        ...
        if (recordId) {
            ...
            var tranDateObj = currentRecordObj.getValue({ fieldId: 'trandate' });
            
            var formattedTranDate = '';
            if (tranDateObj) {
                var d = new Date(tranDateObj);
                var day = ('0' + d.getDate()).slice(-2);
                var month = ('0' + (d.getMonth() + 1)).slice(-2);
                var year = d.getFullYear();
                formattedTranDate = day + '/' + month + '/' + year;
            }
            
            var currentDateObj = new Date();
            var curDay = ('0' + currentDateObj.getDate()).slice(-2);
            var curMonth = ('0' + (currentDateObj.getMonth() + 1)).slice(-2);
            var curYear = currentDateObj.getFullYear();
            var formattedPickDate = curDay + '/' + curMonth + '/' + curYear;

            var suiteletURL = url.resolveScript({
                scriptId: 'customscript_pick_list_wizard_sut',
                deploymentId: 'customdeploy_pick_list_wizard_sut',
                params: {
                    docType: 'salesorder',
                    document: recordId,
                    subsidiary: subsidiary,
                    location: location,
                    dateFrom: formattedTranDate,
                    dateTo: formattedTranDate,
    ...
    return {
        ...
        callPickListSuitelet: callPickListSuitelet
    };
});