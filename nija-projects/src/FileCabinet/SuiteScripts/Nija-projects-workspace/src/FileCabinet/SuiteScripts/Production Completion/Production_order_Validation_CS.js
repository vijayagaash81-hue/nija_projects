 ...
 /*  function pageInit(scriptContext) {
  ...
  function pageInit(scriptContext) {
    ...
    try {
        ...
        let mappedAccount = accountMap[division];
        if (mappedAccount){
          rec.setValue({
          ...
          log.debug("Mapped Account For Division", mappedAccount);
        }
        // === 2. SET Sales Order Reference ===
        if (soid) {
            rec.setValue({
        ...
        if (soid) {
            ...
            division=rec.getValue('class') || location;
...
function loadSalesOrderData(rec, soid) {
    try {
        ...
        const soProjectType = salesOrderRec.getFieldValue('class');
        ...
        let mappedAccount = accountMap[soProjectType];
        if (mappedAccount){
          rec.setValue({
            fieldId:'custrecord_njt_acnt',
            value: mappedAccount,
        ...
        // C5. Division/Project Type (class)
        if (soProjectType) {
            rec.setValue({
    ...
    function fieldChanged(context) {
      ...
      if (fieldName === 'custrecord_njt_pro_ord_devision') {
            ...
            if (division) {
                let account = accountMappingDivision(division); 
                rec.selectNewLine({ sublistId: "recmachcustrecord_njt_pro_2" });
                rec.setCurrentSublistValue({
                    sublistId: "recmachcustrecord_njt_pro_2",
    ...
    function productionCompletion() {
        const id = currentRecord.get().id;
        const url = `/app/common/custom/custrecordentry.nl?rectype=767&workOrderid=${id}`;
        window.open(url, '_blank');
    }
...
function intercomReq() {
  ...
  function accountMappingDivision(division){
    const accountMap = {
          1: 1697,
          2: 1695,
          3: 1698
