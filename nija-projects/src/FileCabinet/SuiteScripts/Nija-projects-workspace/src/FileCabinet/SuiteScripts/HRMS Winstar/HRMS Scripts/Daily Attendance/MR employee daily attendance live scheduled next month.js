...
var monthToCreate = nextMonth;


log.debug('Next Month', nextMonth);
log.debug('Next Year', nextYear);
 */
/**
 * Get next month date safely (no overflow)
 */
var today = new Date(); // or your specific date
//today='2026-01-31T05:22:25.634Z'
//var today = new Date("2026-01-31T05:22:25.634Z");
log.debug('Current Date', today);

// Clone the date so the original isn't changed
var nextMonthDate = new Date(today);

// Step 1: Move to the 1st of current month to prevent overflow
nextMonthDate.setDate(1);

// Step 2: Increase month by 1
nextMonthDate.setMonth(today.getMonth() + 1);

// Step 3: Get last day of that next month
var lastDayOfNextMonth = new Date(
    nextMonthDate.getFullYear(),
    nextMonthDate.getMonth() + 1,
    0
);

// Step 4: If today's date exists in next month, keep it;
// otherwise, use last day of that month
var dayToSet = Math.min(today.getDate(), lastDayOfNextMonth.getDate());
nextMonthDate.setDate(dayToSet);

// Step 5: Format for NetSuite
var formattedNextMonth = format.format({
    value: nextMonthDate,
    type: format.Type.DATE
});
...
log.debug('Next Month Date', formattedNextMonth);
log.debug('Next Month', nextMonth);
log.debug('Next Year', nextYear);
var dateString=formattedNextMonth;
