        ...
        try {
            ...
            if (context.type === context.UserEventType.CREATE || context.type === context.UserEventType.EDIT) {
                ...
                if (id && approvalSts == 2 && dailycheck==false) {
                    ...
                    /*    if (!existingInTime) {
                           // If in time is not present, set the in time and out time from the new record
                           attendanceRecord.setValue('custrecord_njt_emp_daily_in_time', regularIn);
                           attendanceRecord.setValue('custrecord_njt_emp_daily_out_time', regularOut);
   
                           // Calculate work hours
                           //caluclateWorkHours( regularIn, regularOut);
                       } else if (existingInTime && !existingOutTime) {
                           // If only in time is present, set the out time from the new record
                           attendanceRecord.setValue('custrecord_njt_emp_daily_out_time', regularOut);
