 ...
 */
      ...
      log.debug("today",today);
        ...
        var month = today.getMonth() + 1; // getMonth() gives 0-11, so add 1
var year = today.getFullYear();
var yearId = getyearid(year);
var monthToCreate = month;
        log.debug({ title: 'Formatted Current Date', details: dateString }); */
      var now = new Date();

    // 1. Get the full Date/Time string based on the user's timezone preference
    // This will return something like "01/05/2026 09:35:02 am"
    var dateTimeString = format.format({
        value: now,
        type: format.Type.DATETIMETZ 
    });

    // 2. Extract just the date part (the part before the space)
    // Result: "01/05/2026"
    var dateString = dateTimeString.split(' ')[0]; 

    // 3. Extract Day, Month, and Year correctly from the adjusted string
    ...
    log.debug('Extracted Local Date', dateString);
    log.debug('Month/Year', month + ' / ' + year);
        // Construct SuiteQL query with multiple employee IDs
/* 
        if (employeeIds) {
            var sqlQuery =
                "SELECT id, hiredate FROM employee WHERE custentity_hris_emp_employeecheck='T' and hiredate <= '" +
                dateString +
                "' AND id IN ('" +
                employeeIds.join("', '") +
                "')";
                ...
                var sqlQuery =
                "SELECT id, hiredate FROM employee WHERE custentity_hris_empemploymentstatus=1 and isinactive='F' and  hiredate <= '" +
                dateString +
                "' ";
         /* var sqlQuery =
    "SELECT id, hiredate FROM employee WHERE custentity_hris_empemploymentstatus=1 and isinactive='F' and id=1025 and hiredate <= '" +
    dateString +
    "' ";  */
     ...
     * @param {number} employeeId - The employee ID.
     */
    function setDutyRosterAttendance(attendanceRecord, year, month, employeeId) {
        var daysInMonth = new Date(year, month, 0).getDate();
        var startDay = 1;
     ...
     * @param {number} employeeId - The employee ID.
     */
    function setweeklyoffvalue(attendanceRecord, yearId, monthId, employeeId,parentRecordId) {
  ...
  var setsqlquery =
            ...
            "custentity_hris_empholidays AS holidays FROM employee AS B INNER JOIN customrecord_njt_emp_daily_atten_ch AS A " +
            "ON B.id = A.custrecord_njt_daily_atten_emp WHERE B.id = " +
            employeeId +"and  TO_CHAR(A.custrecord_njt_emp_daily_date, 'YYYY') = '"+yearId+"' AND EXTRACT(MONTH FROM A.custrecord_njt_emp_daily_date) = "+monthId+" and A.custrecord_njt_emp_daily_parent ="+parentRecordId+""; 
    ...
    function setpublicholidaysfunc(
        ...
        var publiholsql =
            "SELECT custrecord_hris_holiday_date FROM customrecord_hris_holiday_master where isinactive='F'";
        ...
        if (recordsforpublichol && recordsforpublichol.length > 0) {
            for (var k = 0; k < recordsforpublichol.length; k++) {
               ...
               var setsqlqueryholiday =
                    "SELECT id FROM customrecord_njt_emp_daily_atten_ch WHERE custrecord_njt_emp_daily_date = '" +holidayDate +"' AND custrecord_njt_emp_daily_parent = " +parentRecordId + " AND custrecord_njt_daily_atten_emp = " +employeeId;
                   


  /*              var setsqlqueryholiday = "select b.id as id \
     from customrecord_njt_emp_daily_attendance a join customrecord_njt_emp_daily_atten_ch b \
     on a.id = b.custrecord_njt_emp_daily_parent where a.id ="+parentRecordId+" and a.custrecord_njt_emp_daily_date = '" +holidayDate +"'";
 */
            ...
            for (var j = 0; j < childAttenDetails.length; j++) {
              ...
              /*   if (
               ...
               if (
                    ...
                    attenChildId &&
                    setAttenId != weeklyoffAttenTypeId &&
                    setAttenId != publicHolAttenTypeId && unpaidcheck =='F'
                ) {
                    log.emergency("Processing approved leave for update");
                    try {
                        var attendanceChildRecord = record.load({
