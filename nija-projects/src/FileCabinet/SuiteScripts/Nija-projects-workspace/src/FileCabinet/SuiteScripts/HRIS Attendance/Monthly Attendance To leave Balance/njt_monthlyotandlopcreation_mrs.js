    ...
    function (record, search, task, log, format, runtime, url, query) {
        ...
        function getInputData() {
            try {
                ...
                log.debug({ title: 'Visa Checking Data', details: scriptParams.getParameter({ name: 'custscript_njt_monthlyattend1' }) });

                return JSON.parse(scriptParams.getParameter({ name: 'custscript_njt_monthlyattend1' }));
            }
        ...
        function createMonthlyAttendanceRecord(data, query, format) {
            try {
                ...
                for (var loop = 0; loop < tsResult.length; loop++) {
                    ...
                    var presentdays = rec.custrecord_hrms_month_presentdays || 0;
                    ...
                    if (wageDetails) {
                        ...
                        var payDate = wageDetailsArray[0];
                    ...
                    //     var salaryAmt = 0;
                    //     var otsequence = 1;
                    //     var paycompData = getPayrollcomponent(paygroup, otsequence);
                    //     var paycomponent = paycompData ? paycompData.id : null;
                    //     var overHr = paycompData ? paycompData.hours : 0;
                    ...
                    // }
                    var compoffdays = parseFloat(data.compOff) || 0;
                    if (compoffdays > 0) {
                        try {
                            var leavebalancesql = "SELECT id, custrecord_hris_lvbal_leave_balance_cred, custrecord_hris_lvbal_available_leave_ba, custrecord_hris_lvbal_annual_leave_bal FROM customrecord_hris_leavebalance WHERE custrecord_hris_lvbal_employee_name = " + empid + " AND custrecord_hris_lvbal_leave_type = 11 AND isinactive = 'F'";
                            var leavebalRecords = getResult(leavebalancesql);
                            if (leavebalRecords.length > 0) {
                                var leavebalid = leavebalRecords[0].id;
                                var leavebalancecredited = parseFloat(leavebalRecords[0].custrecord_hris_lvbal_leave_balance_cred) || 0;
                                var availableleavebal = parseFloat(leavebalRecords[0].custrecord_hris_lvbal_available_leave_ba) || 0;
                                var annualleavebal = parseFloat(leavebalRecords[0].custrecord_hris_lvbal_annual_leave_bal) || 0;

                                var newCredited = leavebalancecredited + compoffdays;
                                var newAvailable = availableleavebal + compoffdays;
                                var newAnnual = annualleavebal + compoffdays;

                                record.submitFields({
                                    type: 'customrecord_hris_leavebalance',
                                    id: leavebalid,
                                    values: {
                                        // 'custrecord_hris_lvbal_leave_balance_cred': newCredited,
                                        // 'custrecord_hris_lvbal_annual_leave_bal': newAnnual,
                                        'custrecord_hris_lvbal_available_leave_ba': newAvailable
                                    }
                                });
                                log.debug('Updated leave balance record ID: ' + leavebalid, 'Added Comp-Off Days: ' + compoffdays);
                            } else {
                                log.error('Leave Balance Not Found', 'No Compensatory Off leave balance record found for Employee ID: ' + empid);
                            }
                        } catch (err) {
                            log.error('Error updating leave balance', err.toString());
                        }
                    }
                    if (data.parId) {
                        record.submitFields({
        ...
        function getPayrollcomponent(paygroup, otsequence) {
            try {
                ...
                var recordspayrollcomp = getResult(paycompsql);
                log.debug("recordspayrollcomp", recordspayrollcomp);
                return recordspayrollcomp.length > 0 ? {
                    id: recordspayrollcomp[0].id,
                    hours: recordspayrollcomp[0].custrecord_hris_overtime_hours
                } : null;
            } catch (e) {
                log.error('Error in getPayrollcomponent', e);
                return null;
        ...
        function getbasicpay(empid) {
            try {
                var empcompensql = "SELECT COALESCE(SUM(b.custrecord_hris_cde_monthly), 0) AS monthlyamt, " +
                    ...
                    "GROUP BY c.custrecord_hris_overtime_hours";
