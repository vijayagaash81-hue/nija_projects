function suitelethris(request, response) {
	try {
		...
		if (request.getMethod() == 'GET') {
            ...
            nlapiLogExecution('DEBUG', 'paygroup', paygroup);
            ...
            var dateParts = stdate.split("/"); // Split the date into parts
            var month = dateParts[1]; // Get the month
            ...
            nlapiLogExecution('DEBUG', 'FirstEndDate', FirstEndDate);
			var LastEndDate = new Date(enddate.getFullYear(), parseInt(enddate.getMonth()) + 1, 0);
            nlapiLogExecution('DEBUG', 'LastEndDate', LastEndDate);
 ...
 */
    ...
    var fileName = month + "_" + year + "_" + employee + "_Pay_Slip.pdf";
                  
   /*  // Set the content type and output PDF to the response (for inline display)
    response.setContentType("PDF", "Pay_Slip " + empl_name + ".pdf", "inline");
    response.write(file.getValue());

    // Now, save the PDF in the File Cabinet
    var fileName = "Pay_Slip_" + empl_name + ".pdf"; // Dynamic file name */
    response.setContentType('PDF', fileName, 'inline');
    response.write(file.getValue());
    var fileObj = nlapiCreateFile(fileName, 'PDF', file.getValue());
    fileObj.setFolder(527); // Specify the folder ID where the file will be saved
    fileObj.setIsOnline(true);
    var fileId = nlapiSubmitFile(fileObj); // Save the file to the File Cabinet

    nlapiLogExecution('DEBUG', 'File Saved', 'File ID: ' + fileId);

    // Get the current record's internal ID (e.g., the record that triggered the Suitelet)
    //var recordId = currentRecord.id; // Make sure 'currentRecord' is defined in the Suitelet context

    // Now, call the Map/Reduce script and pass the 'fileId' and 'recordId'
    var scriptId = 'customscript_hris_psr_generation_1_0_sch'; // Replace with your Map/Reduce script ID
    var deploymentId = 'customdeploy_hris_psr_generation_1_0_sch'; // Replace with your Map/Reduce deployment ID

    // Schedule the Map/Reduce script with the necessary parameters
    var params = {
        'custscript_file_id_': fileId,
        'custscript_payslip_record_id_': recordId
    };

    try {
        // Schedule the Map/Reduce script to run asynchronously
        var scheduledScriptId = nlapiScheduleScript(scriptId, deploymentId, params);
        nlapiLogExecution('DEBUG', 'Map/Reduce Script Scheduled', 'Script ID: ' + scriptId + ', Deployment ID: ' + deploymentId + ', Scheduled Script ID: ' + scheduledScriptId);
    } catch (e) {
        nlapiLogExecution('ERROR', 'Error Scheduling Map/Reduce Script', e.message);
    }
...
// Check remaining usage of governance units
var context = nlapiGetContext();
var RemainingUsage = context.getRemainingUsage();
