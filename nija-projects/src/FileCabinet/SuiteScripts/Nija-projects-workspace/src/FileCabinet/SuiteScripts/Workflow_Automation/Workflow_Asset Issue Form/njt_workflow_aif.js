...
define(['N/search', 'N/record', 'N/log', 'N/runtime', 'N/email'], (search, record, log, runtime, email) => {
    ...
    const FIELD_MAPPINGS = {
        status: 'custrecord_approval_status_aif',
        approval_setup: 'custrecord_approval_setup_aif',
        current_level: 'custrecord_current_level_aif',
        next_approver: 'custrecord_next_approver',
        next_approver_role: 'custrecord_next_approval_role_aif',
        rejection_reason: 'custrecord_reason_for_rejection_aif',
        approval_requestor: 'custrecord_approval_requestor_aif',
        department: 'custrecord_hris_asset_issues_form_dept_n',
        email_recipient_field: 'custrecord_hris_asset_employee_name',
        email_cc: [
        ...
        email_template_approved: {
            title: 'Asset Issues Form',
            subject: 'Asset Issues Form - {tranid}',
            body: '<p>Dear {custrecord_hris_asset_employee_name},</p>' +
                '<p>Your Asset Issues Form is approved.</p>' +
                '<p>Employee Name: {custrecord_hris_asset_employee_name}<br />' +
                'Department: {custrecord_hris_asset_issues_form_dept_n}</p>' +
                '<p>Thank you</p>'
        },
        email_template_rejected: {
            ...
            subject: 'Asset Issues Form - {tranid}',
            body: '<p>Dear {custrecord_hris_asset_employee_name},</p>' +
                '<p>Your Asset Issues Form is Rejected.</p>' +
                '<p>Employee Name: {custrecord_hris_asset_employee_name}<br />' +
                'Department: {custrecord_hris_asset_issues_form_dept_n}</p>' +
                '<p>Thank you</p>'
        },
    ...
    const beforeLoad = (context) => {
        ...
        try {
            ...
            var approvalStatusVal = rec.getValue({
                fieldId: FIELD_MAPPINGS.status
            });

            var isPending = approvalStatus === 'Pending Approval' || approvalStatusVal == '1' || approvalStatusVal == 1;
            var isApproved = approvalStatus === 'Approved' || approvalStatusVal == '2' || approvalStatusVal == 2;
            var isRejected = approvalStatus === 'Rejected' || approvalStatusVal == '3' || approvalStatusVal == 3;

            var approvalSetup = rec.getValue({
                fieldId: FIELD_MAPPINGS.approval_setup
            });
            var isCustomRecord = rec.type.indexOf('customrecord_') === 0;
            if ((isPending || isRejected) && !isCustomRecord) {
            ...
            log.debug('Status Text', approvalStatus);
            log.debug('Status Val', approvalStatusVal);
            ...
            if (bannerText) {
                ...
                bannerField.defaultValue =
                    ...
                    "'" + bannerText + "'" +
                    "+ '</span>';" +
                    "if (jQuery('.uir-page-title-secondline').length) {" +
                    "  jQuery('.uir-page-title-secondline').append(banner);" +
                    "} else if (jQuery('.uir-page-title').length) {" +
                    "  jQuery('.uir-page-title').append(banner);" +
                    "}" +
                    "});" +
                    "</script>";
            };
