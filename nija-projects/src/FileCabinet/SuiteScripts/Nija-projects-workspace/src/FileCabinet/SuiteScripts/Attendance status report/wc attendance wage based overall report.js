 ...
 *  - DYNAMIC: Wage period fetched from customrecord_hris_attendance_report_wage based on Pay Group.
...
function (serverWidget, log, query, runtime, url, file) {
    ...
    function onRequest(context) {
        ...
        log.debug('Calculated Wage Period Result', dr ? { 
            start: dr.start.toDateString(), 
            end: dr.end.toDateString(), 
            totalDays: dr.days.length 
        } : 'No wage period found matching Pay Group: ' + payGrp + ', Month: ' + monthId + ', Year: ' + yearId);
    ...
    function calculateWagePeriod(monthId, yearId, payGrp) {
        ...
        var sql = "SELECT TO_CHAR(custrecord_hris_attendance_report_startd, 'DD/MM/YYYY') as start_date, " +
                  "TO_CHAR(custrecord_hris_attendance_report_end, 'DD/MM/YYYY') as end_date " +
                  "FROM customrecord_hris_attendance_report_wage " +
                  "WHERE custrecord_hris_attenda_report_paygroup = ? AND custrecord_hris_attendance_report_month = ? AND custrecord_hris_attendance_report_year = ?";
    ...
    function getAttendanceData(subId, dr) {
        ...
        log.debug('getAttendanceData Parameters', {
            subsidiaryId: subId,
            startDate: fmt(dr.start),
            endDate: fmt(dr.end)
        });
        ...
        try {
            var pagedResults = query.runSuiteQLPaged({
                query: sql,
                params: [subId, fmt(dr.start), fmt(dr.end)],
                pageSize: 1000
            });
            ...
            log.error('Error running runSuiteQLPaged, falling back to runSuiteQL', queryErr);
            results = query.runSuiteQL({ query: sql, params: [subId, fmt(dr.start), fmt(dr.end)] }).asMappedResults();
        }
        
        log.debug('Attendance Records Found Count', results.length);

        // Fetch all active employees in this subsidiary to identify who is missing logs
        try {
            var allEmpSql = "SELECT id, entityid, custentity_hris_empcode AS emp_code FROM employee WHERE subsidiary = ? AND isinactive = 'F'";
            var allEmps = query.runSuiteQL({ query: allEmpSql, params: [subId] }).asMappedResults();
            log.debug('Total Active Employees in Subsidiary', allEmps.length);

            var foundEmpCodes = {};
            results.forEach(function(row) {
                if (row.emp_code) {
                    foundEmpCodes[row.emp_code] = true;
                }
            });

            var missingEmps = [];
            allEmps.forEach(function(e) {
                if (!foundEmpCodes[e.emp_code]) {
                    missingEmps.push(e);
                }
            });

            log.debug('Active Employees Missing Daily Logs in Results', {
                count: missingEmps.length,
                missingList: missingEmps.map(function(e) { return { code: e.emp_code, name: e.entityid }; })
            });

            if (missingEmps.length > 0) {
                var diagnosticList = [];
                missingEmps.forEach(function(emp) {
                    var diag = { code: emp.emp_code, name: emp.entityid, status: '' };
                    try {
                        var parentCheckSql = "SELECT id FROM CUSTOMRECORD_NJT_EMP_DAILY_ATTENDANCE WHERE custrecord_njt_emp_atten_employee = ?";
                        var parentRes = query.runSuiteQL({ query: parentCheckSql, params: [emp.id] }).asMappedResults();
                        if (parentRes.length === 0) {
                            diag.status = "Missing Parent Attendance Record (CUSTOMRECORD_NJT_EMP_DAILY_ATTENDANCE)";
                        } else {
                            var parentId = parentRes[0].id;
                            var childCheckSql = "SELECT COUNT(id) AS cnt FROM CUSTOMRECORD_NJT_EMP_DAILY_ATTEN_CH " +
                                                "WHERE custrecord_njt_emp_daily_parent = ? AND custrecord_njt_emp_daily_date BETWEEN TO_DATE(?, 'DD/MM/YYYY') AND TO_DATE(?, 'DD/MM/YYYY')";
                            var childRes = query.runSuiteQL({ query: childCheckSql, params: [parentId, fmt(dr.start), fmt(dr.end)] }).asMappedResults();
                            var cnt = parseInt(childRes[0].cnt || 0);
                            if (cnt === 0) {
                                diag.status = "Parent record exists (ID: " + parentId + "), but has 0 daily logs in this date range.";
                            } else {
                                diag.status = "Has " + cnt + " logs in date range. Check for code/name mismatch.";
                            }
                        }
                    } catch (diagErr) {
                        diag.status = "Error checking records: " + diagErr.message;
                    }
                    diagnosticList.push(diag);
                });
                log.debug('Diagnostic for Missing Employees', diagnosticList);
            }
        } catch (e) {
            log.error('Error debugging active employees', e);
        }
