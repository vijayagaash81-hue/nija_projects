...
define([
  ...
  "N/query",
  "N/runtime"
], function (search, record, log, format, moment, query,runtime) {
  ...
  function getInputData(context) {
    ...
    var previousDate = '02/02/2026'
      var scriptParams = runtime.getCurrentScript().getParameter({ name: 'custscript_hris_biotimeemp' });
    ...
    try {
      ...
      log.error({ title: 'Parameter Parse Error', details: 'Failed to parse custscript_hris_biotimeemp: ' + e.toString() });
      return [];
    }

    if (!Array.isArray(params) || params.length === 0) {
      log.error({ title: 'Invalid Parameters', details: 'custscript_hris_biotimeemp is empty or not an array' });
      return [];
    }

    // Extract parameters
    var param = params[0]; // Assuming single object as per provided format
  //  var biodate = param.biodate
    var fromdate = param.fromdate;
    var todate = param.todate;
    var subsidiary = param.subsidiary
...
var employeeIds = param.employee
   ...
   var customrecord_njt_daily_attendance_bioSearchObj = search.create({
      ...
      filters: [
        [
          ["custrecord_hris_bio_attendancemethod", "anyof", "1"],
           "AND",
          ["custrecord_njt_bio_date", "within", fromdate,todate],
          "AND",
          ["custrecord_njt_employee_name","anyof",employeeIds],
                   
                        "AND", 
                         ["custrecord_njt_employee_name.subsidiary","anyof",subsidiary]
        ...
        if (searchResult.length > 0) {
            ...
            recordsData.forEach(function (recordData) {
               ...
               // var internalAttendanceType = (minBioTime) ? 1 : null;
                ...
                /* if (internalAttendanceType !== null) {
